...
export const getLists = async (dispatch) => {
    ...
    try {
      const res = await axiosInstance.get("/lists", {   //get all lists from DB
        headers: {
...
export const createList = async (list, dispatch) => {
    ...
    try {
        const res = await axiosInstance.post("/lists", list, {  //create new list in DB 
            headers: {
                token: "Bearer" + JSON.parse(localStorage.getItem('user')).accessToken,
            }
        })
...
export const deleteList = async (id, dispatch) => {
    ...
    try {
        await axiosInstance.delete("/lists/" + id, {    //delete list by ID from DB
            headers: {
                token: "Bearer" + JSON.parse(localStorage.getItem('user')).accessToken,
            }
        })
...
export const updateList = async (id, dispatch, list) => {
    ...
    try {
        const res = await axiosInstance.put("/lists/" + id, list, { //update list from DB
            headers: {
                token: "Bearer" + JSON.parse(localStorage.getItem('user')).accessToken,
            }
        })
